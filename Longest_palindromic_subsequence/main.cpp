/*
Figure this out if the max should be applied to the palindrome
condition arr[i]==arr[j] where we are adding two to the length.
So should that be included or not.
*/
#ifndef _UTILS
...
int main() {
  string arr = "A";
  int result = longest_seq(arr);
  cout <<"The longest sequence length is "<<result;
  getchar();
	return 0;
}
